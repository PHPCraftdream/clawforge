...
import { checksumOf, checksumOfFileMap } from "../framework/checksums.ts";
import { canonicalJson } from "../framework/set/model.ts";
import type { SetManifest } from "../framework/set/model.ts";
import { setDiff } from "../framework/commands/set-diff.ts";
import { diffManifests } from "../framework/set/diff.ts";
...
try {
  ...
  await withOutputSink(() => {}, async () => {
    const { unpackArtifactVerified } = await import("../framework/set/install.ts");
