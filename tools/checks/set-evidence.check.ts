...
import { accept } from "../framework/commands/accept.ts";
import { useDeployment, deploymentDir } from "../framework/deployment.ts";
import { listReceipts, readReceipt } from "../framework/set/receipt.ts";
import { withOutputSink } from "../framework/output.ts";
import type { Context } from "../framework/context.ts";
import type { RunOneOffOptions } from "../framework/runtime.ts";
...
const ctx = {
  ...
  runtime: {
    ...
    runOneOff: async (_service: string, args: string[], options: RunOneOffOptions) => {
