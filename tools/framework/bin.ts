...
import { runGateCommand, gateHelpLines, type GateCommand } from "./gate.ts";
import { reportError } from "./log.ts";
import { useDeployment } from "./deployment.ts";
...
// and the MCP tool all come from one place; see framework/gate.ts. `check` is absent on
