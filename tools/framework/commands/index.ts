...
import type { AppCommand } from "../app.ts";

import { status } from "./status.ts";
import { up, down, logs, restart } from "./lifecycle.ts";
import { cli } from "./cli.ts";
import { cliStart, cliStop } from "./cli-helper.ts";
import { bootstrap } from "./bootstrap.ts";
import { configureProvider } from "./provider.ts";
import { applyConfig } from "./config.ts";
import { backup } from "./backup.ts";
import { restore } from "./restore.ts";
import { verify } from "./verify.ts";
import { pull, push } from "./state.ts";
import { mcpServe, mcpSetup, mcpCreds } from "./mcp.ts";
import { deploy } from "./deploy.ts";
import { inspect, doctor } from "./inspect.ts";
...
import { PROFILES } from "../archive.ts";
...
export const openclawCommands: Record<string, AppCommand> = {
  ...
  "configure-provider": {
    summary: "Configure a model provider from the key in config/.env",
    run: configureProvider,
    details:
      "Reads the key from <data>/config/.env on the target and runs OpenClaw's onboarding " +
      "with --secret-input-mode ref,\n" +
      "so the key itself never enters openclaw.json — only a reference to the environment " +
      "variable does.\n" +
      "That is what makes a deployment reproducible: drop the same key into config/.env on " +
      "any target and this reaches the same result.\n" +
      "bootstrap calls this itself; running it again is a no-op unless --force is given.",
    arguments: [
      { name: "force", description: "Re-run even if already configured", kind: "flag" },
    ],
  },
  secrets: {
