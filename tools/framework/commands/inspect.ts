...
import { deploymentName, desiredStateFile, recipesDir } from "../deployment.ts";
import { status as secretStatus } from "../secrets.ts";
import { openclawCliJson, openclawCli } from "../openclaw-cli.ts";
import { recipeFileChecksums, agentBundleChecksums } from "../checksums.ts";
import { compareLock, readLock, currentComposition, frameworkVersion as lockFrameworkVersion } from "./lock.ts";
import { readInstalledSet, requirementProblems } from "../set/install.ts";
import { readLedger, orphanedBy, foreign } from "../set/ledger.ts";
import type { DeclaredOwnership } from "../set/ledger.ts";
import {
  recipeMirrorTargetDir,
  agentWorkspaceTargetDir,
  loadRecipeAgentBundle,
  cronJobMatches,
} from "./provision-agent.ts";
import type { RecipeAgentBundle, AgentConfig, CronJob } from "./provision-agent.ts";
import {
  problem,
  blockingProblems,
  isHealthy,
...
import type { Problem, Inspection, DeclaredState, ObservedState } from "../inspection.ts";
