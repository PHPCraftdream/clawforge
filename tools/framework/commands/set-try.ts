...
import { parseEnv, frameworkRoot } from "../env.ts";
import { useDeployment, deploymentDir, envFile } from "../deployment.ts";
import { createContext } from "../context.ts";
import type { Context } from "../context.ts";
import { mountPoints } from "../mounts.ts";
import { useSetSource, clearSetSource, setSourceDir, withSetSource } from "../set/source.ts";
import { recordInstalledSet, unpackArtifactVerified } from "../set/install.ts";
import type { SetManifest } from "../set/model.ts";
import { validateSet } from "../set/validate.ts";
import { localSecretValues } from "./set.ts";
import { ensureDataDirs, ensureSecretsFile, secretsFileOnTarget, runMaybePrivileged } from "../datadir.ts";
import { ensureBaselineConfig, configureProvider } from "./provider.ts";
import { applyConfig } from "./config.ts";
import { preflightSecrets } from "./secrets.ts";
import { down } from "./lifecycle.ts";
import { provisionAgent } from "./provision-agent.ts";
import { runCheck, requiresModel, summarize, acceptanceSpecError } from "./accept.ts";
import type { AcceptanceResult } from "./accept.ts";
import { observeRuntime, runtimeMatches, saveEvidence } from "../set/evidence.ts";
import type { ObservedRuntime } from "../set/evidence.ts";
...
async function unpackForTry(artifact: string): Promise<VerifiedArtifact> {
...
export async function setTry(ctx: Context, args: string[], dependencies: {
  ...
  try {
    ...
    const modulePath = (name: string) => {
      const path = relative(tempDir, resolve(frameworkRoot, `${name}${extension}`)).replaceAll("\\", "/");
